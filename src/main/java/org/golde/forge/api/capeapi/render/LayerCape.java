...
public class LayerCape implements LayerRenderer
{
    ...
    public LayerCape(RenderPlayer playerRendererIn)
    {
        ...
        System.out.println(AConstants.LOG + "Created LayerCape (playerRenderer=" + playerRendererIn + ")");
    }

    public void doRenderLayer(AbstractClientPlayer entitylivingbaseIn, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch, float scale)
    {
        if (entitylivingbaseIn.hasPlayerInfo() && !entitylivingbaseIn.isInvisible() && entitylivingbaseIn.isWearing(EnumPlayerModelParts.CAPE) && Loader.hasCape(entitylivingbaseIn.getGameProfile().getId()))
        {
			...
			if(!EricsCapeAPI.getInstance().getWhatSHouldRender()[0]) {
				return;
			}
			
            GlStateManager.color(1.0F, 1.0F, 1.0F, 1.0F);
            this.playerRenderer.bindTexture(Loader.getCape(entitylivingbaseIn.getGameProfile().getId()));
